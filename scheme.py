...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operator1 = scheme_eval(expr.first, env)
        operands1 = expr.rest.map(lambda i: scheme_eval(i, env))
        return scheme_apply(operator1, operands1, env)
...
def eval_all(expressions, env):
    ...
    if expressions == nil: 
        return None
    EvaluatedExp = scheme_eval(expressions.first, env)
...
def do_and_form(expressions, env):
    ...
    if exp01 == nil:
        return True
    while exp01.rest is not nil:
        exp01val = scheme_eval(exp01.first, env)
        if is_false_primitive(exp01val):
            return exp01val
        exp01 = exp01.rest
    return scheme_eval(exp01.first, env)
...
def do_or_form(expressions, env):
    ...
    6
    """
    exp02, exp02val = expressions, False
    if exp02 == nil:
        return False
    while exp02 is not nil:
        exp02val = scheme_eval(exp02.first, env)
        if is_true_primitive(exp02val):
            return exp02val
        exp02 = exp02.rest
    return exp02val
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            if clause.rest == nil: #tests if sub-expression exists, if not return pred
                return test
            else:
                return eval_all(clause.rest, env) #if subexpressions exists, use eval_all to get last element
...
def make_let_frame(bindings, env):
    ...
    names, values, start_pair = nil, nil, bindings
    while start_pair is not nil:
        validate_form(start_pair.first, 2, 2)
        names, values  = Pair(start_pair.first.first, names), Pair(scheme_eval(start_pair.first.rest.first, env), values)
        start_pair = start_pair.rest
